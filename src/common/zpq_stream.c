...
IoStreamProcessor zpq_processor = {
	...
	.destroy = (io_stream_destroy_func) zpq_free
};
...
struct ZpqStream
{
	...
	pg_compress_algorithm compress_algs[COMPRESSION_ALGORITHM_COUNT];	/* array of compression
...
zpq_enable_compression(ZpqStream * zpq, pg_compress_algorithm *algorithms, size_t n_algorithms)
{
	...
	for (int i = 0; i < COMPRESSION_ALGORITHM_COUNT; i++)
	{
		if (i < n_algorithms)
		{
...
zpq_toggle_compression(ZpqStream * zpq, char msg_type, uint32 msg_len)
{
	...
	if (should_compress)
	{
		 ...
		 */
		if (zpq->compress_alg != new_compress_alg)
		{
...
zpq_write(IoStreamLayer * self, ZpqStream * zpq, void const *src, size_t src_size, size_t *bytes_written)
{
	...
	*bytes_written = src_pos;
	...
	while (zpq_buf_unread(&zpq->tx_out))
	{
		...
		{
			/*
			 * Clear internal buffers on permanent failure, as upper layers will flush
			 * and tracking of msg_type/msg_len will otherwise get out of sync
			 */
...
zpq_process_switch(ZpqStream * zpq)
{
	...
	/*
	 * if the new decompressor does not match the current one, change out the
	 * underlying z_stream
	 */
	if (algorithm != zpq->decompress_alg)
	{
...
zpq_parse_compression_setting(const char *val, pg_compress_specification *compressors, size_t *n_compressors)
{
	...
	*n_compressors = 0;
		...
		pg_strcasecmp(val, "1") == 0)
	{
		...
		/*
		 * return all available compressors, processing NONE (0) as the last
		 * algorithm rather than the first
		 */
		for (i = 1; i <= COMPRESSION_ALGORITHM_COUNT; i++)
		{
			if (supported_compression_algorithm(i % COMPRESSION_ALGORITHM_COUNT))
				*n_compressors += 1;
		}
		...
		for (i = 1; i <= COMPRESSION_ALGORITHM_COUNT; i++)
		{
			if (supported_compression_algorithm(i % COMPRESSION_ALGORITHM_COUNT))
			{
				parse_compress_specification(i % COMPRESSION_ALGORITHM_COUNT, NULL, &compressors[j]);
				j += 1;
			}
		}
		return 1;
...
zpq_deserialize_compressors(char const *c_string, pg_compress_specification *compressors, size_t *n_compressors)
{
	...
	*n_compressors = 0;
	...
	while (*p != '\0')
	{
		...
		{
			parse_compress_specification(algorithm, col == NULL ? NULL : col + 1, spec);
			...
			if (spec->options & PG_COMPRESSION_OPTION_WORKERS || spec->options & PG_COMPRESSION_OPTION_LONG_DISTANCE)
			{
				pg_log_warning("streaming compression does not support workers or long distance options");
				goto error;
			}
	...
	/*
	 * Always add NONE to the list at the end to allow one-directional
	 * compression, but only if there are other algorithms available. If there
	 * are no active compressors available, we should return n_compressors = 0
	 * to allow other layers to skip adding the compression processing layers
	 */
	if (n_compressors > 0 && !(selected_alg_mask & (1 << PG_COMPRESSION_NONE)))
	{
		pg_compress_specification *spec = &compressors[*n_compressors];
...
/*
 * Because deserialize rejects all options, this method only needs to concern
 * itself with only serializing name and level
 */
char *
zpq_serialize_compressors(pg_compress_specification const *compressors, size_t n_compressors)
{
	...
	for (i = 0; i < n_compressors; i++)
	{
		 ...
		 */
		total_len += strlen(get_compress_algorithm_name(compressors[i].algorithm)) + level_len + 2;
	}
	...
	for (i = 0; i < n_compressors; i++)
	{
		p += sprintf(p, "%s:%d", get_compress_algorithm_name(compressors[i].algorithm), compressors[i].level);
